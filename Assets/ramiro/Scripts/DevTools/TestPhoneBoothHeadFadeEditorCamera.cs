...
/// <summary>
/// Editor fly camera for TestPhoneBoothHeadFade — move the HMD in/out of the booth interior volume.
/// </summary>
public class TestPhoneBoothHeadFadeEditorCamera : MonoBehaviour
{
#if UNITY_EDITOR
    const string LogPrefix = "[TestPhoneBoothHeadFadeEditorCamera]";
    ...
    public static void EnsureOnMainCamera()
    {
        ...
        if (cam == null)
            return;
        if (cam.GetComponent<TestPhoneBoothHeadFadeEditorCamera>() != null)
            return;

        cam.gameObject.AddComponent<TestPhoneBoothHeadFadeEditorCamera>();
    }
    ...
    void Update()
    {
        ...
        {
            loggedControls = true;
            ConfigManager.WriteConsole($"{LogPrefix} RMB+mouse=look | WASD=move | Shift=fast | Q/E=up-down");
        }
