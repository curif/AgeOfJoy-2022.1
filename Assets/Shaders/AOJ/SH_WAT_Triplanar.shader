Shader "AgeOfJoy/WorldAligned_Triplanar"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                float2 uv;
                if (abs(IN.worldNormal.y) > max(abs(IN.worldNormal.x), abs(IN.worldNormal.z)))
                {
                    uv = scaledWorldPos.xz; // Surface facing Y axis (up or down)
                }
