...
#define INIT_AND_COPY_STRING(dest, src) do { \
...
int wrapper_environment_open(wrapper_log_printf_t _log, 
  ...
  handlers.retro_set_environment = (void (*)(retro_environment_t))dlsym(handlers.handle, 
  ...
  {  
    wrapper_environment_log(RETRO_LOG_ERROR,
                          "[wrapper_enwrapper_environment_openvironment_init] retro_get_system_info not found\n");
    wrapper_dlclose();
    return -1;
  }
  ...
  {  
    wrapper_environment_log(RETRO_LOG_ERROR,
                          "[wrapper_environment_open] retro_init not found\n");
    wrapper_dlclose();
    return -1;
  }
  ...
  {  
    wrapper_environment_log(RETRO_LOG_ERROR,
                          "[wrapper_environment_open] retro_deinit not found\n");
    wrapper_dlclose();
    return -1;
  }
  ...
  wrapper_environment_log(RETRO_LOG_INFO,
                          "[wrapper_environment_open] dlsym retro_unload_game\n");
  ...
  {  
    wrapper_environment_log(RETRO_LOG_ERROR,
                          "[wrapper_environment_open] retro_set_controller_port_device not found\n");
    wrapper_dlclose();
    return -1;
  }
  handlers.retro_set_video_refresh = (void (*)(retro_video_refresh_t))dlsym(handlers.handle,
...
void wrapper_environment_init() {
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_init] retro_set_environment callback\n");
...
void wrapper_environment_get_av_info() {
  ...
  if (av_info.geometry.base_width < 0 || av_info.geometry.base_width > 1024 ||
    ...
    wrapper_environment_log(RETRO_LOG_ERROR, "[wrapper_environment_get_av_info] inconsistence detected / memory corruption\n");
  }
...
double wrapper_environment_get_sample_rate() {
  // wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_get_sample_rate]\n");
...
bool wrapper_environment_cb(unsigned cmd, void *data) {
  ...
  switch (cmd) 
  {
    ...
    case RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY:
      ...
      if (!data)
        return false;
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY]: %s\n", system_directory);
    ...
    case RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY:
      ...
      if (!data)
        return false;
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY]%s\n", save_directory);
    ...
    case RETRO_ENVIRONMENT_SET_ROTATION:
      ...
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_SET_ROTATION] %i\n", *ptr);
      return true;
    ...
    case RETRO_ENVIRONMENT_SET_CONTROLLER_INFO:
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_SET_CONTROLLER_INFO] last environment call from retro_load_game\n");
      return false;

    case RETRO_ENVIRONMENT_GET_CAN_DUPE:
      if (!data)
        return false;
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_GET_CAN_DUPE]\n");
      *(bool*)data = true;
      return true;

    case RETRO_ENVIRONMENT_SHUTDOWN:
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_SHUTDOWN] ???\n");
      return false;

    // case RETRO_ENVIRONMENT_GET_PREFERRED_HW_RENDER:
    //   wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_GET_PREFERRED_HW_RENDER] VULKAN\n");
...
{
    wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_retro_deinit] retro_deinit\n");
    if (!handlers.handle)
      return;
...
void wrapper_unload_game() {
  ...
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_unload_game] retro_unload_game\n");
