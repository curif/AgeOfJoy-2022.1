...
{
  //is preferable to send a string than many pointers to unknown variable types (fail often)
  va_list args;
...
void wrapper_environment_init(retro_log_printf_t log, char *_save_directory, char *_system_directory, char *_sample_rate)
{
  memset(&handlers, 0, sizeof(struct handlers_struct));
  pixel_format = RETRO_PIXEL_FORMAT_UNKNOWN;

  handlers.log = log;
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_init] start -----------\n");
  handlers.handle = dlopen("mame2003_plus_libretro_android.so", RTLD_LAZY);
  handlers.retro_set_environment = (void (*)(retro_environment_t))dlsym(handlers.handle, "retro_set_environment");
  handlers.retro_get_system_av_info = (void (*)(struct retro_get_system_av_info*))dlsym(handlers.handle, "retro_get_system_av_info");
  ...
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_init] sample_rate: %s\n", brightness);
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_init] save_directory: %s\n", save_directory);
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_init] system_directory: %s\n", system_directory);
...
void wrapper_environment_set_game_parameters(char *_gamma, char *_brightness)
{
  ...
  wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_set_game_parameters] gamma: %s brightness: %s \n", gamma, brightness);
}
...
void wrapper_environment_log_geometry()
{
  char *fmt = "[wrapper_environment_log_geometry] Geo:\n"
              ...
              "    max_height: %i\n"
              "    aspect_ratio: %f\n";
  wrapper_environment_log(RETRO_LOG_INFO, fmt, av_info.geometry.base_width, av_info.geometry.base_height, av_info.geometry.max_width, av_info.geometry.max_height, av_info.geometry.aspect_ratio);
}
...
bool wrapper_environment_cb(unsigned cmd, void *data)
{
  ...
  switch (cmd) 
  {
    case RETRO_ENVIRONMENT_GET_VARIABLE:
      ...
      #ifdef ENVIRONMENT_DEBUG
      wrapper_environment_log(RETRO_LOG_INFO, "[wrapper_environment_cb] get var: %s", var->key);
      #endif /* ifdef ENVIRONMENT_DEBUG */
    ...
    case RETRO_ENVIRONMENT_SET_PIXEL_FORMAT:
      ...
      wrapper_environment_log(RETRO_LOG_INFO, "[RETRO_ENVIRONMENT_SET_PIXEL_FORMAT] %i\n", pixel_format);
      return true;
    ...
    case RETRO_ENVIRONMENT_SET_GEOMETRY:
      if (!data)
        return false;
      memcpy(&av_info.geometry, (struct retro_game_geometry*)data, sizeof (struct retro_game_geometry));
