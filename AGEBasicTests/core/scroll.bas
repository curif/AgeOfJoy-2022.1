...
70    PRINTLN "ORIGINAL LINE # " + STR(LINEIDX), 0, 0 : REM Print lines, deferring display. PRINTLN manages its own Y. DRAW_FLAG as string "0"
80 NEXT LINEIDX
...
150 PRINT 0, ScreenHeight() - 5, "--- NEW TEXT ON SCROLLED UP (1) ---", 0, "0"
160 PRINT 0, ScreenHeight() - 4, "--- NEW TEXT ON SCROLLED UP (2) ---", 0, "0"
170 PRINT 0, ScreenHeight() - 3, "--- NEW TEXT ON SCROLLED UP (3) ---", 0, "0"
180 PRINT 0, ScreenHeight() - 2, "--- NEW TEXT ON SCROLLED UP (4) ---", 0, "0"
190 PRINT 0, ScreenHeight() - 1, "--- NEW TEXT ON SCROLLED UP (5) ---", 0, "0"
...
260 PRINT 0, 0, "--- NEW TEXT ON SCROLLED DOWN (A) ---", 0, "0"
270 PRINT 0, 1, "--- NEW TEXT ON SCROLLED DOWN (B) ---", 0, "0"
280 PRINT 0, 2, "--- NEW TEXT ON SCROLLED DOWN (C) ---", 0, "0"
...
450 DBOX DCHARPIXEL(CORNER[0], CORNER[1]), 
        ARRAY(BOXPWIDTH, BOXPHEIGHT), 
        YELLOW , 0, 0 : REM Draw border only, defer display. 0 for FILL_FLAG, 0 for DRAW_FLAG. Removed FILL_COLOR_SPEC.

460 REM Fill the rectangle with initial content using explicit PRINT X,Y
470 FOR LINEIDX = 0 TO CORNER[1] - 1
480    PRINT CORNER[0], CORNER[1] + LINEIDX, "RECT CONTENT LINE " + STR(LINEIDX) , 0, "0"
490 NEXT LINEIDX
...
570 PRINT CORNER[0], CORNER[1] + SIZE[1] - 2, "NEW RECT LINE UP (1)", 0, 0
580 PRINT CORNER[0], CORNER[1] + SIZE[1] - 1, "NEW RECT LINE UP (2)", 0, 0
...
630 SCROLLRECT CORNER, SIZE, 1, YELLOW : REM Scroll rect down 1 line, fill new space with magenta. DRAW_FLAG omitted to defer.
640 REM Print new text on the line that appeared at the top of the rectangle using explicit PRINT X,Y
650 PRINT CORNER[0], CORNER[1], "NEW RECT LINE DOWN (TOP)", 0, 0
660 SHOW : REM Display scrolled rectangle content and new line
670 SLEEP 3

680 CLS : REM Clear final screen
700 END