...
void update_variables(bool first_time)
{
  ...
  //log_cb(RETRO_LOG_INFO, LOGPRE "curif update variables, first time: %i\n", (int)first_time);

  for(index = 0; index < OPT_end; index++)
  {
    var.value = NULL;
    var.key = default_options[index].key;
    //log_cb(RETRO_LOG_INFO, LOGPRE "curif  #%i GET VARIABLE %s\n", var.key);
    ...
    {
      //log_cb(RETRO_LOG_INFO, LOGPRE "curif #%i GET VARIABLE %s: frontend returns %s\n", index, var.key, var.value);
      switch(index)
      {
        case OPT_INPUT_INTERFACE:
          log_cb(RETRO_LOG_INFO, LOGPRE "curif input interface %s\n", var.value);
        ...
        case OPT_4WAY:
          if( (strcmp(var.value, "enabled") == 0) && (options.content_flags[CONTENT_JOYSTICK_DIRECTIONS] == 4) )
          {
            if(!options.restrict_4_way)           /* the option has just been toggled to "enabled" */
            {
              ...
              reset_control_descriptions = true;  /* games with rotated joysticks send diaferent control descriptions in 4-way restrictor mode */
            }
          }
          else
          {
        ...
        case OPT_SKIP_DISCLAIMER:
          log_cb(RETRO_LOG_INFO, LOGPRE "curif SKIP disclaimer %s\n", var.value);
          if(strcmp(var.value, "enabled") == 0)
        ...
        case OPT_SKIP_WARNINGS:
          log_cb(RETRO_LOG_INFO, LOGPRE "curif SKIP WARNINGS %s\n", var.value);
          if(strcmp(var.value, "enabled") == 0)
