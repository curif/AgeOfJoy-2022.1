...
3.  **Fast Load:** If valid, the raw ETC2/ASTC byte array is read directly from the `.aojv1` file using a `BinaryReader`, pushed into a new `Texture2D` via `LoadRawTextureData()`, and sent straight to the GPU. This bypasses the heavy PNG/JPG decoding entirely.
